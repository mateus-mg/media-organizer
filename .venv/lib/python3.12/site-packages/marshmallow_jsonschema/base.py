...
def _resolve_additional_properties(cls) -> bool:
    ...
    if unknown is None:
        ...
        raise UnsupportedValueError(f"Unknown value {unknown} for `unknown`")
...
class JSONSchema(Schema):
    ...
    def get_required(self, obj) -> list[str] | typing.Any:
    ...
    def _from_python_type(self, obj, field, pytype) -> dict[str, typing.Any]:
        ...
        # Support both marshmallow 3.x (default) and 4.x (dump_default)
        field_default = getattr(
            field, "dump_default", getattr(field, "default", missing)
        )
        if field_default is not missing and not callable(field_default):
            json_schema["default"] = field_default
    ...
    def _get_python_type(self, field):
        ...
        raise UnsupportedValueError(f"unsupported field type {field}")
    ...
    def _from_nested_schema(self, obj, field):
        ...
        if field_default is not missing and not callable(field_default):
            schema["default"] = nested_instance.dump(field_default)

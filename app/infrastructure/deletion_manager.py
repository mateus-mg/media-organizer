...
Usage:
    from app.infrastructure import DeletionManager, LinkRegistry, TrashManager
    from pathlib import Path
    
    # Data directory configurable via MEDIA_ORG_DATA_DIR environment variable (default: data/)
...
class DeletionManager:
    ...
    async def delete_to_trash(
        ...
        try:
            ...
            log_success(
                self.logger, f"Moved to trash: {path} (ID: {trash_id})")
            return DeletionResult(
                success=True,
                operation_type='trash',
                trash_id=trash_id,
    ...
    async def delete_permanent(
        ...
        try:
            ...
            if dry_run:
                log_info(
                    self.logger, f"[DRY-RUN] Would permanently delete: {path}")
                return DeletionResult(
                    success=True,
                    operation_type='dry_run',
                    removed_links=[link['path']
            ...
            if self.organization_database:
                ...
                if backup_path:
                    log_info(
                        self.logger, f"Database backup created: {backup_path}")
            ...
            for link in preview['all_links']:
                ...
                if link_path.exists():
                    try:
                        ...
                        log_error(
                            self.logger, f"Failed to remove {link_path}: {e}")
                        return DeletionResult(
                            success=False,
                            operation_type='permanent',
    ...
    def _confirm_permanent_deletion(self, preview: Dict[str, Any]) -> bool:
        ...
        console.print()
        console.print(
            "[bold red][WARNING] PERMANENT DELETION WARNING[/bold red]")
        console.print()
        console.print(
            "The following files will be [bold red]PERMANENTLY DELETED[/bold red]:")
        console.print()
        ...
        console.print()
        console.print(
            f"Total size: [yellow]{preview['total_size_display']}[/yellow]")
        console.print()
        console.print("[bold red]This action CANNOT be undone.[/bold red]")
        console.print(
            "All hard links will be removed and disk space will be freed.")
        console.print()
    ...
    def _confirm_trash_deletion(self, preview: Dict[str, Any]) -> bool:
        ...
        console.print()
        console.print(
            f"Total size: [cyan]{preview['total_size_display']}[/cyan]")
        console.print()
        console.print(
            f"A copy will be stored in trash for {self.trash_manager.retention_days} days.")
        console.print()
    ...
    def _update_organization_database(self, path: Path, deletion_type: str):
        ...
        try:
            ...
            log_info(
                self.logger, f"Updated organization database: {deletion_type}")

        except Exception as e:
            log_error(
                self.logger, f"Failed to update organization database: {e}")
    ...
    async def restore_from_trash(self, trash_id: str, restore_paths: Optional[List[Path]] = None) -> bool:
        ...
        try:
            ...
            # Restore
            success = self.trash_manager.restore_from_trash(
                trash_id, restore_paths)
    ...
    def print_preview(self, preview: Dict[str, Any]):
        ...
        for link in preview['all_links']:
            ...
            console.print(
                f"  [cyan]•[/cyan] {link_path} [dim]({link_type})[/dim]")

        console.print()
        console.print(f"Total links: [bold]{preview['total_links']}[/bold]")
        console.print(
            f"Total size: [bold]{preview['total_size_display']}[/bold]")
        console.print()

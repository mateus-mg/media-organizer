...
class OrganizationDatabase(DatabaseInterface):
    ...
    BACKUP_ARTIFACTS = [
        ("organization_", "organization.json"),
        ("invalid_music_genres_", "invalid_music_genres.json"),
        ("suspect_music_genres_", "suspect_music_genres.json"),
        ("link_registry_", "link_registry.json"),
        ("artist_genre_cache_", "artist_genre_cache.json"),
        ("genre_exceptions_", "genre_exceptions.json"),
        ("musical_keywords_", "musical_keywords.json"),
        ("genre_guard_cycle_report_", "genre_guard_cycle_report.json"),
        ("genre_guard_decisions_latest_", "genre_guard_decisions_latest.json"),
        ("genre_guard_rule_suggestions_", "genre_guard_rule_suggestions.json"),
        ("trash_index_", "trash/index.json"),
    ]
    ...
    def get_record_by_original_path(self, file_path: str) -> Optional[Dict[str, Any]]:
        """Return media record by original source path, if present."""
        Media = Query()
        return self.media_table.get(Media.original_path == str(file_path))
    ...
    def create_backup(self) -> Optional[Path]:
        ...
        try:
            ...
            for prefix, relative_path in self.BACKUP_ARTIFACTS:
                src = data_dir / relative_path
                if not src.exists():
                    continue
                dst = self.backup_dir / f"{prefix}{timestamp}.json"
    ...
    def cleanup_old_backups(self):
        ...
        for prefix, _ in self.BACKUP_ARTIFACTS:
            for backup_file in self.backup_dir.glob(f"{prefix}*.json"):
                try:
                    timestamp_str = backup_file.stem.replace(prefix, "")
                    timestamp = datetime.strptime(
                        timestamp_str, "%Y-%m-%d_%H-%M-%S")
                    if timestamp < cutoff:
                        backup_file.unlink()

...
Usage:
    from app.infrastructure import LinkRegistry
    from pathlib import Path
    
    # Path can be configured via MEDIA_ORG_DATA_DIR environment variable (default: data/)
...
# Global locks indexed by absolute database path to synchronize
# multiple LinkRegistry instances writing to the same TinyDB file.
...
def _get_db_lock(db_path: Path) -> threading.RLock:
    """Get or create a lock for the given database path."""
...
class LinkRegistry:
    ...
    def __init__(self, db_path: Path):
        ...
        self.db_lock = _get_db_lock(self.db_path)

        # Initialize TinyDB - will be reopened before each write to sync _max_id
        self.db = None
    ...
    def _open_db(self):
        """Open/reopen database connection to sync internal state."""
        self.db = TinyDB(str(self.db_path), indent=2, ensure_ascii=False)
        self.files_table = self.db.table('files')

    def _sync_before_write(self):
        """Reopen database before write to sync TinyDB's internal _max_id counter."""
        try:
    ...
    def _restore_from_latest_backup(self) -> bool:
        """Restore link registry from latest valid backup when missing."""
        backup_dir = self.db_path.parent / "backups"
        if not backup_dir.exists():
            return False

        backup_files = sorted(
            backup_dir.glob("link_registry_*.json"),
    ...
    def register_link(
        ...
        with self.db_lock:
            try:
                ...
                if not inode:
                    log_error(
                        self.logger, f"Cannot get inode for: {source_path}")
                    return False
                ...
                if existing:
                    ...
                    log_debug(
                        self.logger, f"Updated link registry for inode {inode}")
                    ...
                    log_info(
                        self.logger, f"Registered new inode {inode} with {len(record['links'])} links")
    ...
    def unregister_link(self, path: Path) -> bool:
        ...
        try:
            with self.db_lock:
                ...
                new_links = [link for link in links if link.get(
                    'path') != path_str]
                ...
                if len(new_links) == 0:
                    ...
                    log_info(
                        self.logger, f"Removed inode record {inode} (no more links)")
                else:
    ...
    def _unregister_by_path(self, path: Path) -> bool:
        ...
        Note: This is called from inside unregister_link lock context,
        so it reuses the already-synced database connection.
        """
        try:
            with self.db_lock:
                File = Query()
    ...
    def scan_filesystem(
        ...
        log_info(
            self.logger, f"Starting filesystem scan: {len(directories)} directories")
                ...
                for file_path in directory.rglob('*'):
                    if file_path.is_file():
                        ...
                        try:
                            ...
                            stats['errors'] += 1
                            log_warning(
                                self.logger, f"Cannot stat file: {file_path}")
                ...
                if len(paths) > 1:  # Only register files with multiple links
                    ...
                    if not existing:
                        ...
                        try:
                            ...
                            stats['errors'] += 1
                            log_error(
                                self.logger, f"Failed to register inode {inode}: {e}")

        log_success(
            self.logger, f"Scan complete: {stats['inodes_registered']} inodes registered")
        return stats
    ...
    def cleanup_invalid_links(self) -> int:
        ...
        try:
                ...
                for record in all_records:
                    ...
                    for link in record.get('links', []):
                        ...
                        if path.exists():
                            ...
                            removed_count += 1
                            log_info(
                                self.logger, f"Removed invalid link: {path}")
            ...
            log_info(
                self.logger, f"Cleanup complete: {removed_count} invalid links removed")
            return removed_count

        except Exception as e:
            log_error(self.logger, f"Failed to cleanup invalid links: {e}")
            return 0
